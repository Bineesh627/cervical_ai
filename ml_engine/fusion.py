...
# src/fusion.py
def fuse_probs(p_clin, p_img, w_clin=0.5, w_img=0.5):
    p_clin = float(p_clin or 0.0); p_img = float(p_img or 0.0)
    s = w_clin + w_img or 1.0
    return (w_clin*p_clin + w_img*p_img) / s
...
def fuse_probs(clin_prob, img_prob, w_clin=0.6, w_img=0.4):
...
def run_multimodal_prediction(image_or_path, clinical_features_dict):
    ...
    prediction_label = "Abnormal" if fused > 0.005 else "Normal"
