...
def prepare_clinical(df, target="Biopsy"):
    ...
    # Convert binary/categorical columns that are read as floats/ints to objects
    # to ensure they are handled by OneHotEncoder (common issue with this dataset)
    cols_to_convert = ['Smokes', 'Hormonal Contraceptives', 'IUD', 'STDs', 'Dx:Cancer', 'Dx:CIN', 'Dx:HPV', 'Dx', 'Hinselmann', 'Schiller', 'Citology']
    for col in cols_to_convert:
        if col in X.columns:
            # Fill missing with mode/a placeholder before converting to object for OHE
